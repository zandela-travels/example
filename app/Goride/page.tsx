...
import { LanguageTypes, Towns, VehicleTypes } from '@/constants'; // Import VehicleTypes here
...
const Page = () => {
  ...
  const handleFilter = () => {
    ...
    if (selectedLanguage !== 'Any') {
      filtered = filtered.filter((driver) => driver.language === selectedLanguage);
    }
    ...
    <>
      ...
      <div className="bg-backgroundImg bg-cover bg-no-repeat w-screen flex flex-col items-center">
        ...
        <Dialog open={isFilterDialogOpen} onOpenChange={setIsFilterDialogOpen}>
          <DialogContent className='text-dark-200 bg-white-500'>
            <DialogHeader>
            ...
            <div className="mt-4">
              ...
              <select
                id="location"
                className="mt-2 w-full p-2 border rounded text-dark-200"
              ...
              <label htmlFor="language" className="block text-sm font-medium mt-4">Driver Language Preferance</label>
              <select
                id="language"
                className="mt-2 w-full p-2 border rounded text-dark-200"
              ...
              <button
                className="mt-4 px-4 py-2 blue-btn text-white-500 rounded transition duration-300"
        ...
        <section className="w-11/12 lg:w-4/5 my-10 flex flex-col lg:flex-row">
          ...
          <div className="border-blue hidden lg:block lg:w-1/4 bg-white-500 p-5 rounded-[10px] shadow-lg h-fit">
            ...
            <h2 className="text-lg font-semibold mb-4 mt-4">Filter by Driver Language Preferance</h2>
              <select
                id="language"
                className="mt-2 w-full p-2 border rounded text-dark-200"
                value={selectedLanguage}
            ...
            <button
              className="mt-4 px-4 py-2 blue-btn rounded transition duration-300"
              onClick={handleFilter}
            >
              Apply Filter
            </button>
          ...
          <button
            className="mb-4 px-4 py-2 bg-footerImg text-white-500 rounded lg:hidden hover:bg-gray-900 transition duration-300"
          ...
          <div className="bg-transparent backdrop-blur-[3px] remove-scrollbar scroll-smooth h-[700px] overflow-y-scroll px-2 rounded-lg bg-transparent lg:w-4/5">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 rounded-lg">
              {filteredDrivers.map((driver) => (
                <div
                  key={driver.userId}
                  className="border-blue bg-white-500 rounded-[10px] shadow-lg p-4 text-center hover:shadow-xl transition duration-300"
                >
                  <Image
                  ...
                  <button
                    className="mt-6 w-full px-4 py-2 blue-btn rounded transition duration-300"

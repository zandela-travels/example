  ...
  return (
    <div className='flex items-center gap-3 px-0 py-3 md:px-1 md:py-5'>
        <SliderButton handleClick={handlePrev}>
            <IoIosArrowBack className='text-xl text-white-500' />
        </SliderButton>
        <SliderButton handleClick={handleNext}>
            <IoIosArrowForward className='text-xl text-white-500' />
        </SliderButton>
...
const SliderButton = ({
    ...
    return (
        <button
            className='flex h-14 w-14 items-center justify-center rounded-full border-[2px] border-[#fdfdfd5f] transition duration-300 ease-in-out hover:bg-white-500 hover:text-dark-200' onClick={handleClick}
        >
            {children}
        </button>    
    );
